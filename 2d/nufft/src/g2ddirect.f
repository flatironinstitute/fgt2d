...
c
            do ii = 1,nd
               dtmp = rtmp*dipstr(ii,i)
               pot(ii) = pot(ii) + rinc*dtmp
               grad(ii,1) = grad(ii,1) + rincx*dtmp
               grad(ii,2) = grad(ii,2) + rincy*dtmp
            enddo
         endif
      enddo
      return
...
c---------------------------------------------------------------------
      integer i,ns,ifexpon,ii,nd
      real *8 sources(2,ns),rnormal(2,ns),targ(2)
      real *8 xdiff,ydiff,rr,r,rtmp,dtmp
      real *8 rinc,dx,dy,rincx,rincy,delta
...
c---------------------------------------------------------------------
      ...
      real *8 xdiff,ydiff,rr,r,rtmp,dtmp,rinc,dx,dy,rincx,rincy,rfac
      real *8 pot(nd),grad(nd,2),eps,dmax
      real *8 charge(nd,ns),dipstr(nd,ns)
...
c---------------------------------------------------------------------
      ...
      real *8 xdiff,ydiff,rr,r,rtmp,dtmp,rinc,dx,dy,rincx,rincy,delta
