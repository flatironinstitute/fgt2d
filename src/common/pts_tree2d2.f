...
c 
      ...
      subroutine pts_tree_mem(src,ns,targ,nt,idivflag,ndiv,ngrid,nlmax,
...
c    - ngrid: integer
c        number of boxes in each dimension at the root level
c    - nlmax: integer
...
c
      ...
      integer nlmin,iper,ifunif,ngrid
      integer ns,nt,ndiv
...
c
      nbctr = ngrid*ngrid
      call binsort2d(src,ns,targ,nt,ngrid,nbctr,centers,isrcse,
     1   isrc,itargse,itarg,boxsize(0))
      nbctr = ngrid*ngrid
      laddr(1,0) = 1
      laddr(2,0) = nbctr
...
c
     ...
     1        iparent,nchild,ichild,iper,ngrid,nnbors,nbors)

        if(nlevels.ge.2.and.ifunif.ne.1) then
          call pts_tree_fix_lr(centers,nlevels,nboxes,boxsize,nbmax,
     1         nlmax,iper,ngrid,laddr,ilevel,iparent,nchild,ichild,
     2         nnbors,nbors)
        endif
      endif
...
c
      ...
      subroutine pts_tree_build(src,ns,targ,nt,idivflag,ndiv,ngrid,
     1  nlmax,ifunif,iper,nlevels,nboxes,ltree,itree,iptr,
     2  centers,boxsize)
c
c
c----------------------------------------
...
c
      ...
      integer iper,nlmin
      integer iptr(8)
      integer itree(ltree),ier,ifunif
...
c
     ...
     1        itree(iptr(3)),itree(iptr(4)),itree(iptr(5)),iper,ngrid,
     2        itree(iptr(6)),itree(iptr(7)))

      if(nlevels.ge.2.and.ifunif.ne.1) then
         call pts_tree_fix_lr(centers,nlevels,
     1         nboxes0,boxsize,nboxes,nlevels,iper,ngrid,itree(iptr(1)),
     2         itree(iptr(2)),itree(iptr(3)),itree(iptr(4)),
...
c-------------------------------------------------------------      
     ...
     1       boxsize,nbmax,nlmax,iper,ngrid,laddr,ilevel,iparent,nchild,
...
C$OMP END PARALLEL DO      
     ...
     2                   ichild,iper,ngrid,nnbors,nbors)
...
c
     ...
     1   iptr,centers,ngrid,boxsize,ixy,ixyse)
      implicit real *8 (a-h,o-z)
      integer n,ltree,nboxes,nlevels,itree(ltree),iptr(8)
      integer ixy(n),ixyse(2,nboxes),ngrid,nbctr
      real *8 xys(2,n),centers(2,nboxes),xstart,ystart
      real *8 boxsize(0:nlevels)

      nbctr = ngrid*ngrid

      xstart = centers(1,1) - boxsize(0)/2
      ystart = centers(2,1) - boxsize(0)/2


      call pts_binsort2d(n,xys,xstart,ystart,boxsize(0),ngrid,nbctr,
     1  ixy,ixyse)
...
c
c
      subroutine binsort2d(src,ns,targ,nt,ngrid,nboxes,centers,isrcse,
     1   isrc,itargse,itarg,boxsize)
c
c   This subroutine bin sorts a collection a sources and targets
c   in two dimensions into an ngrid \times ngrid collection
c   of boxes
c
c   Input arguments:
c     - src: real *8 (2,ns)
c         xy coordinates of the source locations
c     - ns: integer
c         number of sources
c     - targ: real *8 (2,nt)
c         xy coordintes of target locations
c     - nt: integer
c         number of targets
c     - ngrid: integer
c         number of boxes in each dimension
c     - nboxes: integer
c         number of boxes = ngrid*ngrid
c   
c   Output arguments:
c     - centers: real *8 (2,nboxes)
c         xy coordinates of the box centers at the root level
c     - isrcse: integer (2,nboxes)
c         starting and ending indices of sorted sources in box i
c     - isrc: integer(ns)
c         permutation array of bin sorted sources
c     - itargse: integer(2,nboxes)
c         starting and ending indices of sorted targets in box i
c     - itarg: integer(nt)
c         permutation array of bin sorted targets
c    

      implicit real *8 (a-h,o-z)
      integer ns,nt,ngrid,nboxes
      integer isrcse(2,nboxes),isrc(ns),itargse(2,nboxes),itarg(nt)
      real *8 src(2,ns),targ(2,nt),centers(2,nboxes),boxsize
...
c
      ...
      boxsize0 = (xmax - xmin)
      sizey = (ymax -ymin)
      if(sizey.gt.boxsize0) boxsize0 = sizey

      boxsize = boxsize0/(ngrid+0.0d0)
c
c  compute box centers
c  following the quad tree convention, the boxes will be numbered as
c  
c      .
c      .
c      .
c   ngrid + 1   |  ngrid + 2  | ......| 2*ngrid
c   -------------------------------------------
c   1           |  2          | ..... | ngrid
c
c
      xstart = (xmin + xmax)/2 - boxsize0/2 
      ystart = (ymin + ymax)/2 - boxsize0/2 
      do jgrid = 1,ngrid
        do igrid = 1,ngrid
          ibox = (jgrid-1)*ngrid + igrid
          centers(1,ibox) = xstart + (igrid-0.5d0)*boxsize 
          centers(2,ibox) = ystart + (jgrid-0.5d0)*boxsize
        enddo
      enddo

      call pts_binsort2d(ns,src,xstart,ystart,boxsize,ngrid,nboxes,
     1  isrc,isrcse)
      call pts_binsort2d(nt,targ,xstart,ystart,boxsize,ngrid,nboxes,
     1  itarg,itargse)
...
c
c
      subroutine pts_binsort2d(n,xy,xs,ys,bs,ng,nb,ixy,ixyse)
      implicit real *8 (a-h,o-z)
      real *8 xy(2,n),xs,ys,bs
      integer ng,nb,ixy(n),ixyse(2,nb)
      integer, allocatable :: ioffst(:),ixybox(:),ixyboxsort(:)
      integer, allocatable :: itmpxy(:)


      allocate(ioffst(nb+1))
      allocate(ixybox(n),ixyboxsort(n),itmpxy(n))
      ...
      call sorti(n,ixybox,ixy)

      call conv_to_csc(n,nb,itmpxy,ixybox,ioffst,ixyboxsort)

      do i=1,nb
        ixyse(1,i) = ioffst(i)
        ixyse(2,i) = ioffst(i+1)-1
      enddo
